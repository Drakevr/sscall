...
void
speex_jitter_init(SpeexJitter *jitter, void *decoder)
{
	jitter->dec = decoder;
	speex_decoder_ctl(decoder, SPEEX_GET_FRAME_SIZE,
			  &jitter->frame_size);
...
void
speex_jitter_put(SpeexJitter *jitter, void *packet, int len,
		 int timestamp)
{
...
void
speex_jitter_get(SpeexJitter *jitter, spx_int16_t *out,
		 int *current_timestamp __attribute__ ((unused)))
{
	int i;
	int ret;
	...
	if (activity < 30)
		jitter_buffer_update_delay(jitter->packets,
					   &packet, NULL);
	jitter_buffer_tick(jitter->packets);
}
